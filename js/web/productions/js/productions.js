    ...
    */
	...
	CalcRatingBody: () => {
			...
			for (const building of ratedBuildings) {
				...
				for (const type of Productions.RatingTypes) {
					if (building[type] != undefined) {
						h.push(`<td class="text-right${type=="units" ? " units":""}" data-number="${Math.round(building[type])}">`)
						h.push(`<span class="buildingvalue" ${type=="units" ? `data-original-title="${randomUnits}"`:""}>`+HTML.Format(building[type])+'</span>')
		...
		$('#ProductionsRatingBody').html(h.join('')).promise().done(function () {
			...
			$('#ProductionsRatingBody td.units span').tooltip({html:true});
		});	
