...
FoEproxy.addHandler('CityMapService', 'reset', async (data, postData) => {
  ...
  r.forEach(async (it) => {
    ...
    if (it.state.__class__ !== 'PlunderedState') { return; }
...
let Plunderer = {
  ...
  // Sadly plunder event have no infor about city entity, just collected resources
  ...
  Render: async () => {
    ...
    actions = actions.map(it => {
      ...
      const playerInfo = player ? ({
        ...
        clanName: player.clanName || 'Not a Clan Member',
...
<div class="header">
  ...
  <div class="filter">${filterByPlayerId ? `${i18n('Boxes.Plunderer.filteredByUser')}. <button class="btn btn-default select-player" data-value="">${i18n('Boxes.Plunderer.showAllPlayers')}</button>` : 'All Players'}</div>
</div>
...
`);
 ...
 ${i18n('Boxes.Plunderer.collectedToday')}: <span class="${todaySP ? 'green' : ''}">${todaySP}</span>SP,
 ${i18n('Boxes.Plunderer.thisWeek')}: <span class="${thisWeekSP ? 'green' : ''}">${thisWeekSP}</span>SP,
 ${i18n('Boxes.Plunderer.total')}:  <span class="${totalSP ? 'green' : ''}">${totalSP}</span>SP
`);
  },
  ...
  RenderAction: ({action, isSamePlayer}) => {
    ...
    const date = moment(action.date).format('MM/DD HH:mm');
    const dateFromNow = moment(action.date).fromNow();
    return `
<div class="action-row action-row-type-${action.type}">
  <div class="avatar select-player" data-value="${action.playerId}">
   ${ isSamePlayer ? '' : `<div><img src="${avatar}"/></div>`}
   <span class="type type-${action.type}">${type}</span>
  </div>
...
`;
  ...
  RenderActionContent: (action) => {
    ...
    case Plunderer.ACTION_TYPE_PLUNDERED:
      ...
      return `
<div class="plunder-wrap">
  <div class="name">
   <img class="sabotage" src="${extUrl}css/images/sabotage.png">
   ${BuildingNamesi18n[action.buildId].name}
  </div>
  <div class="plunder-items ${action.important ? 'important' : '' }">
    ${goodsIds.map(id => {
...
`
    ...
    case Plunderer.ACTION_TYPE_BATTLE_SURRENDERED:
      return `
<div class="battle">
  <div>${action.battle.auto ? i18n('Boxes.Plunderer.autoBattle') : `${i18n('Boxes.Plunderer.rounds')}: ${action.battle.round || 'NA'}`}</div>
...
${!isSameStats ? '' : `
      <div class="stats-same-for-all">
        <div class="${attBoost ? 'green' : ''}">Att: ${attBoost}%</div>
        <div class="${defBoost ? 'green' : ''}">Def: ${defBoost}%</div>
      </div>
`}
${army.map(unit => Plunderer.RenderUnit({unit, showDetails: !isSameStats})).join('')}`;
  },
  ...
  RenderUnit: ({unit, showDetails}) => {
    ...
    return `
<div class="unit" title="unit: ${unit.unitTypeId}, HP: ${endHP}/${startHP}, attack boost: ${attBoost}, defend boost: ${defBoost}">
...
${showDetails ? `
      <div class="stats">
      <div class="${!endHP ? 'red' : ''}">${endHP}/${startHP}</div>
      <div class="${attBoost ? 'green' : ''}">${attBoost}%</div>
      <div class="${defBoost ? 'green' : ''}">${defBoost}%</div>
      </div>
    ` : ''}
</div>
`;
