...
let Parts = {
	...
	CalcBody: (NextLevel) => {
		...
		if (PlayerName) h.push((PlayerDict[PlayerID]['Activity'] ? `<span style="position: relative; top: 3px" class="activity activity_${PlayerDict[PlayerID]['Activity']}"></span> `: ``) + `<strong>${MainParser.GetPlayerLink(PlayerID, PlayerName)}</strong>`);
		h.push('</div>');
