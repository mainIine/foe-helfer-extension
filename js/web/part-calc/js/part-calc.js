...
let Parts = {
	...
	CalcBodyPowerLeveling: () => {
		...
		for (let i = MinLevel; i < MaxLevel; i++) {
			...
			//Kopiere alle Plätze in eine globale Variable um sie Anschliessend in die Zwischenablage kopieren zu können.
			Parts.PowerLevelingPlaces[i] = i + ' → ' + (i + 1) + '  P1 ' + P1s[i] + '  P2 ' + P2s[i] + '  P3 ' + P3s[i] + '  P4 ' + P4s[i] + '  P5 ' + P5s[i];
		}
	...
	CopyPowerlevelsToClipboard: (MinLevel, MaxLevel) => {
		...
		helper.str.copyToClipboard(copyString);
	},
};
