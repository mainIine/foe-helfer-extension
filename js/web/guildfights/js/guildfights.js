	...
	DrawProvinces: ()=> {
		...
		ProvinceMap.SVGPaths().forEach((function(i){
			...
			for (let n = 0; n < pD.length; n++){
				...
				if (ProvinceMap.MapCTX.isPointInPath(e.flag.x, e.flag.y)){
	...
	ParsePathToCanvas: (i)=> {
		...
		for (let o = 0; o < i.length; o++){
			switch (i.charAt(o)){
				...
				case "M":
					(e = ProvinceMap.ParseMove(++o, i)), ProvinceMap.MapCTX.moveTo(e.x, e.y), (o = e.index - 1);
					break;

				case "c":
					(s = ProvinceMap.ParseCurve(++o, i)), ProvinceMap.MapCTX.bezierCurveTo(e.x + s.cp1x, e.y + s.cp1y, e.x + s.cp2x, e.y + s.cp2y, e.x + s.x, e.y + s.y), (o = s.index - 1), (e.x += s.x), (e.y += s.y);
					break;

				case "m":
					let c = ProvinceMap.ParseMove(++o, i);
					ProvinceMap.MapCTX.moveTo((e.x = e.x + c.x), (e.y = e.y + c.y)), (o = c.index - 1);
					break;

				case "C":
					(s = ProvinceMap.ParseCurve(++o, i)), ProvinceMap.MapCTX.bezierCurveTo(s.cp1x, s.cp1y, s.cp2x, s.cp2y, s.x, s.y), (e.x += s.x), (e.y += s.y), (o = s.index - 1);
					break;
