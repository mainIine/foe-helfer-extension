	...
	BuildFightContent: () => {
		...
		for(let i in mP)
		{
			...
			{				
				...
				if(mP[i]['ownerId'] === undefined && mP[i]['conquestProgress'].length > 0 && mP[i]['conquestProgress'][0]['participantId'] === bP[x]['participantId']) // wenn Sektoren noch niemandem gehören
				{
						t.push('<tr id=' + cnt + ' data-id="' + cnt + '">');
						t.push('<td>');
						t.push(mP[i]['title']);
		...
		{
			let date = new Date(),
			basictime = 1610000000, // übermittelt von foe
			maxtime = 950400, // 11 Tage GG
			locked = mP[i]['lockedUntil'] - basictime, // sekunden der Gesamtsperrzeit
			...
			for(let x = 0; x < 1; x++)
			{						
					if(mP[i]['lockedUntil'] !== undefined && own['clan']['name'] !== mP[i]['owner'])
					{	
						arraysector.push(sectorfree);
						arrayprov.push(mP[i]);
					}
			}
	}
	...
	prepare: ()=> {
		...
		Province.prototype.drawGGMap = function() {
			...
			if(ProvinceMap.MapCTX.isPointInPath(path, mouse.x + this.flag.x, mouse.y + this.flag.y) && this.lockedUntil){
				...
				ProvinceMap.MapCTX.fillText(moment.unix(this.lockedUntil).format('HH:mm:ss'), mouse.oX + this.flag.x, mouse.oY + this.flag.y);
			}
		}
