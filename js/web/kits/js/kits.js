...
let Kits = {
			...
			if (show) {
				...
				if (Name) { //Name is set
					...
					if (Name === 'Kits') {
						...
						ChainSetIco = '<img src="' + srcLinks('/shared/icons/' + sName + '.png', true) + '" class="chain-set-ico">';
					}
	...
	ItemTd: (el)=> {
		...
		if (el['type'] === 'first') {
			...
			url = srcLinks('/city/buildings/' + [aName.slice(0, 1), '_SS', aName.slice(1)].join('') + '.png', true) ;
		}
			...
			if (aName.includes('fragment')) {
				...
				url = srcLinks('/shared/icons/icon_fragment.png', true);
				url_fragment = `<img class="kits-fragment-image" src="${url}" alt="${item['name']}"/>`;
			}

			url = srcLinks('/shared/icons/reward_icons/reward_icon_' + aName + '.png', true);
