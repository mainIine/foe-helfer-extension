...
let Stats = {
	...
	createUnitsSeries: async () => {
		...
		return {
			...
			pointFormat: `<tr>
								<td>
									<span class="unit_icon {series.options.unitId}" style="background-image:url('${srcLinks("/shared/unit_portraits/armyuniticons_50x50/armyuniticons_50x50_0.png", true)}')"></span>
								</td>
								<td>
									<span style="margin: 0 5px;"><span style="color:{point.color}">●</span> {series.name}: </span>
								</td>
	...
	createRewardSeries: async () => {
		...
		const serieData = Object.keys(seriesMapBySource).map(it => {
			...
			const iconClass = rewardInfo.type === 'unit' ? `unit_icon ${rewardInfo.subType}" style="background-image:url('${srcLinks("/shared/unit_portraits/armyuniticons_50x50/armyuniticons_50x50_0.png", true)}')` : '';
			...
			if (rewardInfo.type != 'unit') {
				...
				if ((rewardInfo.iconAssetName || rewardInfo.assembledReward && rewardInfo.assembledReward.iconAssetName)) {
					...
					url = `${srcLinks("/shared/icons/reward_icons/reward_icon_${icon}.png", true)}`;
					url2 = `${srcLinks("/shared/icons/goods_large/${icon}.png", true)}`;
					...
					if (rewardInfo.type == 'good' && rewardInfo.iconAssetName == 'random_goods' && rewardInfo.subType) {
						url = `${srcLinks("/shared/icons/reward_icons/reward_icon_random_goods.png", true)}`;
					}
					if (rewardInfo.subType == 'fragment' && rewardInfo.subType) {
						if (rewardInfo.assembledReward.type == 'building' && rewardInfo.subType){
							url = `${srcLinks("/city/buildings/${rewardInfo.assembledReward.subType.replace(/^(\w)_/, '$1_SS_')}.png", true)}`;
						}
					}
				}else if (rewardInfo.type == 'building' && rewardInfo.subType) {
						url = `${srcLinks("/city/buildings/${rewardInfo.subType.replace(/^(\w)_/, '$1_SS_')}.png", true)}`;
				}
				if (url) {
