					...
					if (GvGMap.Actions.drag) {
						...
						if (sector.terrain === "plain" || sector.terrain === "beach") {
							if (GvGMap.Marker != false) {
								MapSector.draw(sector, false, GvGMap.Marker);
							}
							else {
...
let MapSector = {
	 ...
	 */
	draw: (sector, redraw = false, marker = false, color = false) => {
		MapSector.drawHex(sector);
		if (color) // for selections
			MapSector.drawHex(sector,color);
		if (marker) {
			MapSector.drawFlagsAndStatus(sector,redraw,marker);	
		}
		if (sector.owner.id > 0 && GvGMap.Size != 'mini' && !marker) 
			MapSector.drawFlagsAndStatus(sector,redraw,marker);			
