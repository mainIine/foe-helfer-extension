...
/**
 *  requestClass: "UpdateService"
	requestMethod: "finishDailyCalculation"
	responseData: true
 */
...
let GvGLog = {
	...
	addEntry: (response) => {
		if (response != undefined && response.type != undefined) {
			...
			else if (response.__class__ == "ClanBattleClanChange") { // headquarter_placed
				// add Guild to pool
				let guildOnMap = {
					id: entry.clan.id,
					name: entry.clan.name,
					flag: entry.clan.flag,
					color: GvGMap.getColor(entry.clan),
					flagCoordinates: GvGMap.getFlagImageCoordinates(entry.clan.flag),
					power: 0,
					sectors: 0,
				};
				GvGMap.Map.Guilds.push(guildOnMap);
